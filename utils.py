...
def zones_nearest_node(zones, nodes, drop_duplicates=False):
    ...
    neigh = nearest(centroid, nodes, n_neighbors=1).rename(
...
def routing(origin, destination, links, weight_col="time", dijkstra_limit=np.inf):
    ...
    dist_matrix = pd.DataFrame(
        _unstack(dist_matrix), columns=["origin", "destination", weight_col]
    )
    # rename origin and destination with original indexes.
    dist_matrix["origin"] = dist_matrix["origin"].apply(
        lambda x: index_node.get(origin_dict.get(x))
    )
    dist_matrix["destination"] = dist_matrix["destination"].apply(
        lambda x: index_node.get(destination_dict.get(x))
    )
    return dist_matrix


def get_catchment_dist(
    link: gpd.GeoDataFrame, catchment_radius: dict, default: float = 500
):
    route_type = link["route_type"].unique()
    if len(route_type) > 1:
        print(
            "multiple route type for a single route_id.. using first one for catchment radius"
        )
...
def nearest_radius(one, many, radius=100, to_crs=None):
    ...
    many = add_geometry_coordinates(
        many, columns=["x_geometry", "y_geometry"], to_crs=to_crs
    )
    one = add_geometry_coordinates(
        one, columns=["x_geometry", "y_geometry"], to_crs=to_crs
    )
...
def population_to_mesh(
    population: gpd.GeoDataFrame,
    mesh: gpd.GeoDataFrame = None,
    step: float = 0.01,
    col: str = "population",
    fill_missing: Literal["centroid", "nearest", None] = "centroid",
) -> gpd.GeoDataFrame:
    """
    create a mesh in the zones total bbox at every step (in the units of the zones crs)
    ...
    population["nodes"] = population["geometry"].apply(
        lambda x: points_in_polygon(points_coords, x)
    )

    nodes = population.reset_index()[["index", "nodes", col]].copy()
    nodes = nodes.explode("nodes").dropna()
    print(
        len(nodes[nodes["nodes"].duplicated()]),
        "nodes in multiple zones. will be match to a single zone.",
    )
    ...
    unfounded_zones = population.loc[~population.index.isin(zones_list)][
        ["geometry", col]
    ]
    if fill_missing == "centroid":
        ...
        unfounded_zones = unfounded_zones.reset_index().rename(
            columns={"index": "zone"}
        )
        ...
        points = points.merge(
            pop_to_append, left_index=True, right_index=True, how="left"
        )
...
def get_acf_distances(
    nodes: gpd.GeoDataFrame, mesh: gpd.GeoDataFrame, crs: int, max_dist: float = 3000
) -> gpd.GeoDataFrame:
    """
    ...
    node_dist = node_dist.rename(
        columns={"ix_one": "node_index", "index_nn": "mesh_index"}
    ).drop(columns="rank")

    nodes_index_dict = nodes.reset_index()["index"].to_dict()
    node_dist["node_index"] = node_dist["node_index"].apply(
        lambda x: nodes_index_dict.get(x)
    )

    node_dist = node_dist.explode(["mesh_index", "distances"])
    population_dict = mesh["population"].to_dict()
    node_dist["population"] = node_dist["mesh_index"].apply(
        lambda x: population_dict.get(x)
    )
    return node_dist


def get_routing_distances(
    nodes: gpd.GeoDataFrame,
    rnodes: gpd.GeoDataFrame,
    rlinks: gpd.GeoDataFrame,
    mesh: gpd.GeoDataFrame,
    weight_col: str = "length",
    dijkstra_limit: float = np.inf,
) -> gpd.GeoDataFrame:
    """
    ...
    mat = routing(
        ...
        dijkstra_limit=dijkstra_limit,
    )

    mat = mat.merge(
        mesh.reset_index()[["index", "node_index", "population"]],
        left_on="destination",
        right_on="node_index",
        how="left",
    )
    mat = mat.drop(columns=["destination", "node_index"]).rename(
